...
#Data for ML
top_20_genes_expression <-  t(top_20_genes_expression)
top_20_genes_expression <- as.data.frame(top_20_genes_expression)
PLT <- c(290,374,240,246,184,409,171,100,29,53,108,186,171,37)
top_20_genes_expression <- cbind(top_20_genes_expression,PLT) 
write.csv(top_20_genes_expression, "top_20_genes_expression.csv", row.names = T)

# Visualize and quality control test results.
# Build histogram of P-values for all genes. Normal test
# assumption is that most genes are not differentially expressed.
tT2 <- topTable(fit2, adjust="fdr", sort.by="B", number=Inf)
hist(tT2$P.Val, col = "grey", border = "white", xlab = "P-adj",
...
palette(c("#1B9E77", "#7570B3", "#E7298A", "#E6AB02", "#D95F02",
                   ...
                   par(mar=c(7,4,2,1))
                   title <- paste ("GSE61853", "/", annotation(gset), sep ="")
                   boxplot(ex[,ord], boxwex=0.6, notch=T, main=title, outline=FALSE, las=2, col=gs[ord])
                   legend("topleft", groups, fill=palette(), bty="n")
                   
                   # expression value distribution
                   par(mar=c(4,4,2,1))
                   title <- paste ("GSE61853", "/", annotation(gset), " value distribution", sep ="")
                   ...
                   plotSA(fit2, main="Mean variance trend, GSE61853")
